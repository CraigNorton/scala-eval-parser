 ...
 * @param binding the list of bindings
 * @param prev the previous environment
 */
private class ExtendedEnvironment(val binding: Binding, val prev: Environment) extends Environment {
  // TODO: Part 4 - implement the extended environment
  def extend(v: Var, a: Value): Environment = new ExtendedEnvironment(new Binding(v,a),this)

  def lookup(v: Var): Option[Binding] = {
    if(this.binding.v == v){Some(binding)}
    else{prev.lookup(v)}
    }

  def toList: List[Binding] = prev.toList :+ binding //this.toList
